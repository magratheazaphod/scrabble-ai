# Causeway Scrabble Challenge 2026 (budak / Cedric Lewis)
**Collection:** Causeway Scrabble Challenge 2026 — Division 3 (2nd lowest) | **Games:** 4 (Rounds 1, 2, 28, 32 only — most rounds not in this collection) | **Record:** 4-0 +199

🟩🟩🟩🟩

## Aggregate Stats (Cedric Lewis / budak)

| Stat | Value |
|---|---|
| Average Mistakes Score | 3.83 |
| Average Score | 450.2 |
| Average Opponent Score | 400.5 |
| Total Bingos | 6 (1.5/game) |
| Bingo Find Rate | 6/10 (60.0%) |
| Opponent Total Bingos | 9 (2.25/game) |
| Total Blanks Drawn | 5 (1.25/game) |
| Avg Endgame Spread Lost | 29.2 |
| Avg Win% Lost | 42.4% |
| Total Phonies Played | 5 |
| Opponent Phonies Played | 1 |
| Games per Missed Bingo | 1.0 |
| Games per Phony Played | 0.8 |

## Per-Game Breakdown

*Notes: `*` marks a phony (all words formed by the play; the specific invalid word isn't always identifiable when multiple words were formed — CSW is the configured lexicon for every game).*

| Rnd | Game | Opponent | Result | Cedric | Opp | Spread | Mistakes | Cedric Bingos | Missed Bingos | Opp Bingos | Cedric Blanks | Endgame Spread Lost | Win% Lost | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| 1 | [↗](https://woogles.io/game/S22b9eB6yHxsXMF9GnfXQ9) | Sultan Alieffany | W | 390 | 367 | +23 | 5.4 | 0 | 1 | 2 | 1 | 54 | 90.1% | errorful win; phony RIFED/RYU* (unchallenged); missed bingo #1 (REFRaI(N)(E)D); Sultan Alieffany phony VAL* |
| 2 | [↗](https://woogles.io/game/jQZwe2c9cCC7J6ULg8mL22) | Amanda Mak | W | 483 | 427 | +56 | 4.6 | 3 | 1 | 2 | 2 | 63 | 32.9% | errorful win; phony URTICAE/DUI* (unchallenged); phony TUMORING*; missed bingo #2 (FLOU(R)ING) |
| 28 | [↗](https://woogles.io/game/FHazSeTPhcM4Ysg9oyqVxf) | Rebecca Soble | W | 509 | 465 | +44 | 2.9 | 1 | 1 | 3 | 1 | 0 | 39.8% | phony EDICTING*; missed bingo #3 (INDI(R)ECT); Rebecca Soble missed O(V)ERLARD |
| 32 | [↗](https://woogles.io/game/PyU3LBGHMxJjrQSbeMEdBx) | Rebecca Soble | W | 419 | 343 | +76 | 2.4 | 2 | 1 | 2 | 1 | 0 | 6.8% | phony MEETE/MA/EN/TI/EE* (unchallenged); missed bingo #4 (OUTWING); Rebecca Soble missed DENSEST |
| **Avg** | | | | **450.2** | **400.5** | **+49.8** | **3.83** | **1.5** | **1.0** | **2.25** | **1.25** | **29.2** | **42.4%** | |

## Missed Bingos

*Lowercase = blank tile; (X) = board tile X was already there*

| Rd | Opponent | Word |
|---|---|---|
| 1 | Sultan Alieffany | REFRaI(N)(E)D |
| 2 | Amanda Mak | FLOU(R)ING |
| 28 | Rebecca Soble | INDI(R)ECT |
| 32 | Rebecca Soble | OUTWING |

## Summary

Cedric went a perfect 4-0 (+199 total spread) across the four games in this collection, though the win came with a relatively high average Mistakes score of 3.83 and 42.4% average win probability lost per game — the wins were earned more on raw output (450.2 average score) than clean play. He found only 6/10 (60.0%) of available bingos and threw 5 phonies across the four games, both worth tightening up. Note this collection only covers Rounds 1, 2, 28, and 32 — most of the tournament isn't represented here.