...
def compute_game(r):
    ...
    opp_fully_annotated = opp_mistake_index is not None and game_is_over  # opponent rack known every turn AND game finished — see note above
...
- **Opponent mistake index / Win% Lost:** only trustworthy when `opp_fully_annotated` is True — opponent's `mistake_index` non-null in `player_summaries` AND the game actually finished (`history["play_state"] == "GAME_OVER"`). Non-null mistake_index alone means the game was annotated for both sides or livestreamed with full rack data, but an incomplete/aborted game can still have one that isn't a meaningful final number. Tournament-level averages (`avg_opp_mi`, `avg_opp_wpl`) must only include those games, not the full collection.
